...
public func ask_feature_set_eq (a: AskFeatureKey, b: AskFeatureKey) : Bool {
  ...
  public func account_to_value (account: Account) : ICRC3.Value {
    switch(account) {
      ...
      case(#account(value)) return #Array([#Blob(Principal.toBlob(value.owner)), switch(value.sub_account) {
          ...
          switch (thisItem) {
            ...
            case (#fee_accounts(value)) {
              ...
              for (account in value.vals()) {
                feeAccounts.add(#Array([#Text(account.0), account_to_value(account.1)]));
              };
  ...
  public func pricing_config_to_value (config: PricingConfigShared) : ICRC3.Value {
    ...
    switch(config) {
      ...
      case(#auction(value)) {
        ...
        newMap.add(("ending", switch(value.ending){
        ...
        newMap.add(("min_increase", switch(value.min_increase){
  ...
  public func candySharedToValue(x: CandyTypes.CandyShared) : ValueShared {
    switch(x){
      ...
      case(#Set(x)) {
        ...
        CandyTypes.CandyShared,ValueShared>(x, func(x: CandyTypes.CandyShared) : ValueShared { candySharedToValue(x)}));
      };
      /* case(#ValueMap(x)) {

...
shared (deployer) actor class test_runner_sale(dfx_ledger : Principal, dfx_ledger2 : Principal) = this {
  ...
  public shared func testReservation() : async { #success; #fail : Text } {
    ...
    let defaultGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = ""; /* default namespace */
    members = null; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000 : ?Nat; symbol = "OGY"; decimals = 8 : Nat; standard = #Ledger; id = null }) }]; allowed_amount = ?2; tier = 0; additive = true })]);

    //create a specific group with allocation of 2 for b wallet
    let aGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = "agroup"; /* default namespace */
    members = ?[Principal.fromActor(a_wallet)]; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000; id = null; symbol = "OGY"; decimals = 8; standard = #Ledger }) }]; allowed_amount = ?1; additive = true; tier = 1 })]);

    //put b and c in b group

    //create a specific group with allocation of 2 for b wallet
    let bGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = "bgroup"; /* default namespace */
    members = ?[Principal.fromActor(b_wallet), Principal.fromActor(c_wallet)]; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000; id = null; symbol = "OGY"; decimals = 8; standard = #Ledger }) }]; allowed_amount = ?2; additive = true; tier = 2 })]);
    ...
    let a_wallet_try_register_for_one = await a_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        max_desired = 1;
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
            ?{
    ...
    let b_wallet_try_register_for_two = await b_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(b_wallet);
        max_desired = 2;
        escrow_receipt = switch (b_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
            ?{
    ...
    let d_wallet_try_register_for_two = await d_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(d_wallet);
        max_desired = 2;
        escrow_receipt = switch (d_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
            ?{
    ...
    //shold be empty
    let d_allocate_empty = await d_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(d_wallet);
    ...
    //should be 2
    let a_allocate_empty = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
    ...
    let a_wallet_try_redeem_for_one = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let b_allocate_empty = await b_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(b_wallet);
        number_to_allocate = 2;
    ...
    let b_wallet_try_redeem_for_one = await b_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (b_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let a_allocate_empty_after_two = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        number_to_allocate = 2;
    ...
    let a_wallet_try_redeem_for_third = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let c_allocate_empty_after_two = await c_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
    ...
    let c_wallet_try_redeem_for_one = await c_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (c_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
            {
              buyer = val.receipt.buyer;
              seller = val.receipt.seller;
    ...
    let c_allocate_empty_2_end = await c_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(c_wallet);
        number_to_allocate = 1;
    ...
    let a_allocate_empty_3_end = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        number_to_allocate = 1;
    ...
    let d_allocate_empty_3_end = await d_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(d_wallet);
        number_to_allocate = 1;
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "can register one item",
          switch (d_balance_before_allocation) {
            case (#ok(res)) {
              if (res.nfts.size() == 0) {
                "expected success";
              } else {
        ...
        S.test(
          "fail d is allocated inventory",
          switch (d_allocate_empty) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5004) {
                //empty inventory because all are allocated to reservations
                "correct number";
              } else {
                "wrong error " # debug_show (err);
              };
        ...
        S.test(
          "fail if a is allocated more than 1 inventory",
          switch (a_allocate_empty) {
            case (#ok(res)) {
        ...
        S.test(
          "b should be allocated two more",
          switch (a_allocate_empty) {
            case (#ok(res)) {
        ...
        S.test(
          "fail d is allocated inventory",
          switch (a_allocate_empty_after_two) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5004) {
                //empty inventory because all are allocated to reservations
        ...
        S.test(
          "fail a if allocating for a un allocated item",
          switch (a_wallet_try_redeem_for_third) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5001) {
                //allocation doesnt exist
                "correct number";
              } else {
                "wrong error " # debug_show (err);
              };
        ...
        S.test(
          "c can register for one",
          switch (c_allocate_empty_after_two) {
            case (#ok(res)) {
              if (res.allocation_size == 1) {
                "expected success";
              } else {
        ...
        S.test(
          "c can register for one",
          switch (c_wallet_try_redeem_for_one) {
            case (#ok(res)) {
              if (res.nfts.size() == 1) {
                "expected success";
              } else {
        ...
        S.test(
          "only 8 items allocated in balance",
          switch (
        ...
        S.test(
          "fail if  is allocated inventory",
          switch (c_allocate_empty_2_end) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5004) {
                //empty inventory because all are allocated to reservations
  ...
  public shared func testReservationNoReg() : async { #success; #fail : Text } {
    ...
    let a_allocate_one = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        number_to_allocate = 1;
    ...
    let b_allocate_two = await b_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(b_wallet);
        number_to_allocate = 2;
    ...
    let b_wallet_try_redeem_for_two = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (b_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let d_allocate_two = await d_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(d_wallet);
        number_to_allocate = 2;
    ...
    let d_wallet_try_redeem_for_two = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (d_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let a_wallet_try_redeem_for_one_more = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
  ...
  public shared func testRegistration() : async { #success; #fail : Text } {
    ...
    let defaultGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = ""; /* default namespace */
    members = null; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000 : ?Nat; symbol = "OGY"; decimals = 8 : Nat; standard = #Ledger; id = null }) }]; allowed_amount = ?5; tier = 0; additive = true })]);

    //create a specific group with allocation of 2 for b wallet
    let bGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = "bgroup"; /* default namespace */
    members = ?[Principal.fromActor(b_wallet)]; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000; id = null; symbol = "OGY"; decimals = 8; standard = #Ledger }) }]; allowed_amount = ?2; additive = true; tier = 1 })]);

    //create a specific group with allocation of 2 for b wallet
    let dGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = ""; /* default namespace */
    members = ?[Principal.fromActor(d_wallet)]; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000; id = null; symbol = "OGY"; decimals = 8; standard = #Ledger }) }]; allowed_amount = ?2; additive = false; tier = 2 })]);
    ...
    let f_wallet_try_registration_before_open = await a_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        max_desired = 1;
    ...
    let a_wallet_try_registration_no_escrow = await a_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        max_desired = 1;
    ...
    let a_wallet_try_registration_low_amount = await a_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        max_desired = 1;
    ...
    let a_wallet_try_redeem_for_one = await a_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        max_desired = 1;
    ...
    let a_wallet_try_register_for_two = await a_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        max_desired = 2;
    ...
    let b_wallet_try_register_for_four = await b_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(b_wallet);
        max_desired = 2;
    ...
    let d_wallet_try_register_for_four = await d_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(d_wallet);
        max_desired = 2;
    ...
    let e_wallet_try_register_for_two = await e_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(e_wallet);
        max_desired = 2;
        escrow_receipt = switch (e_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
            ?{
              buyer = val.receipt.buyer;
              seller = val.receipt.seller;
    ...
    let c_allocate_empty = await c_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(c_wallet);
        number_to_allocate = 1;
    ...
    let c_wallet_try_register_after_sale_date = await c_wallet.try_sale_registration(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(c_wallet);
        max_desired = 1;
    ...
    let c_allocate_empty_2 = await c_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(c_wallet);
        number_to_allocate = 1;
    ...
    let suite = S.suite(
      ...
      [
        S.test(
          "fail if registering before open",
          switch (f_wallet_try_registration_before_open) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5005) {
                //improper escrwo
        ...
        S.test(
          "fail if registering no escrow",
          switch (a_wallet_try_registration_no_escrow) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5003) {
                //improper escrwo
        ...
        S.test(
          "fail if no lock",
          switch (a_wallet_try_registration_bad_lock) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5002) {
                //improper escrwo
        ...
        S.test(
          "fail if not enough tokens",
          switch (a_wallet_try_registration_low_amount) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5003) {
                //improper escrwo
        ...
        S.test(
          "can register one item",
          switch (a_wallet_try_redeem_for_one) {
            case (#ok(res)) {
        ...
        S.test(
          "can get reg afterone item",
          switch (a_wallet_registration_after_one) {
            case (#ok(res)) {
              if (res.max_desired == 1) {
                "expected success";
              } else {
        ...
        S.test(
          "can register two item replace",
          switch (a_wallet_try_register_for_two) {
            case (#ok(res)) {
        ...
        S.test(
          "can get reg after two item",
          switch (a_wallet_registration_after_two) {
            case (#ok(res)) {
              if (res.max_desired == 1) {
                "expected success";
              } else {
        ...
        S.test(
          "can register four item additive",
          switch (b_wallet_try_register_for_four) {
            case (#ok(res)) {
        ...
        S.test(
          "can get reg after four item b",
          switch (b_wallet_registration_after_four) {
            case (#ok(res)) {
              if (res.max_desired == 4) {
                "expected success";
              } else {
        ...
        S.test(
          "can register d four item non additive",
          switch (d_wallet_try_register_for_four) {
            case (#ok(res)) {
              if (res.max_desired == 2) {
                "expected success";
              } else {
        ...
        S.test(
          "can get reg after four item d",
          switch (d_wallet_registration_after_four) {
            case (#ok(res)) {
              if (res.max_desired == 4) {
                "expected success";
              } else {
        ...
        S.test(
          "can register e two item non-additive",
          switch (e_wallet_try_register_for_two) {
            case (#ok(res)) {
              if (res.max_desired == 2) {
                "expected success";
              } else {
        ...
        S.test(
          "can get reg after four item e",
          switch (e_wallet_registration_after_two) {
            case (#ok(res)) {
              if (res.max_desired == 2) {
                "expected success";
              } else {
        ...
        S.test(
          "only 6 items allocated in balance",
          switch (
            a_wallet_balance_after_three,
            b_wallet_balance_after_three,
            d_wallet_balance_after_three,
        ...
        S.test(
          "cannot register after sale ends",
          switch (c_wallet_try_register_after_sale_date) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5005) {
                //improper escrwo
        ...
        S.test(
          "allocation doesnt work inventory cleared during allocation",
          switch (c_allocate_empty) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5004) {
                //improper escrwo
  ...
  public shared func testRedeemAllocation() : async { #success; #fail : Text } {
    ...
    //allocate 5 nfts
    let allocate_1 = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
    ...
    let a_wallet_try_redeem_no_escrow = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = {
          buyer = #principal(Principal.fromActor(a_wallet));
    ...
    let a_wallet_try_redeem_low_amount = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let a_wallet_try_redeem_for_one = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          ...
          case (#err(err)) {
            D.print("THROW ----------------- failed to get escrow for a payment in testRedeem for a_wallet_try_redeem_for_one");
    ...
    let a_wallet_try_redeem_for_two = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let a_wallet_try_redeem_for_ten = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let allocate_2 = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        number_to_allocate = 1;
    ...
    let a_wallet_try_redeem_for_expired = await a_wallet.try_sale_nft_redeem(
      Principal.fromActor(sale_canister),
      {
        escrow_receipt = switch (a_wallet_try_escrow_general_valid) {
          case (#ok(val)) {
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "fail if redeeming with no allocation",
          switch (b_wallet_allocation_attempt) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5001) {
        ...
        S.test(
          "fail if redeeming with a non existant escrow",
          switch (a_wallet_try_redeem_no_escrow) {
            case (#ok(res)) {
        ...
        S.test(
          "fail if the payment was too low",
          switch (a_wallet_try_redeem_low_amount) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5003) {
                //improper low payment
                "correct number";
              } else {
                "wrong error " # debug_show (err);
              };
        ...
        S.test(
          "can allocate one item",
          switch (a_wallet_balance_after_one) {
            case (#ok(res)) {
              if (res.nfts.size() == 1) {
        ...
        S.test(
          "allocation balance after one items",
          switch (a_wallet_allocation_after_one) {
            case (#ok(res)) {
        ...
        S.test(
          "can allocate additive two items",
          switch (a_wallet_allocation_after_three) {
            case (#ok(res)) {
              if (res.allocation_size == 2) {
        ...
        S.test(
          "can allocate additive two items",
          switch (a_wallet_try_redeem_for_two) {
            case (#ok(res)) {
              if (res.nfts.size() == 2) {
        ...
        S.test(
          "has 3",
          switch (a_wallet_balance_after_three) {
            case (#ok(res)) {
              if (res.nfts.size() == 3) {
        ...
        S.test(
          "cannot over allocate  ten items",
          switch (a_wallet_try_redeem_for_ten) {
            case (#ok(res)) {
              if (res.nfts.size() == 2) {
        ...
        S.test(
          "has 3",
          switch (a_wallet_balance_after_ten) {
            case (#ok(res)) {
              if (res.nfts.size() == 3) {
  ...
  public shared func testAllocation() : async { #success; #fail : Text } {
    ...
    //create a specific group with allocation of 2 for b wallet
    let cGroup = await sale_canister.manage_group_sale_nft_origyn([#update({ namespace = "cgroup"; /* default namespace */
    members = ?[Principal.fromActor(c_wallet)]; pricing = ?[#cost_per { amount = 1000000000; token = #ic({ canister = dfx_ledger; fee = ?200000; id = null; symbol = "OGY"; decimals = 8; standard = #Ledger }) }]; allowed_amount = ?2; additive = false; tier = 2 })]);
    ...
    let allocate_0 = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
    ...
    let allocate_2 = await b_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(b_wallet);
    ...
    let allocate_3 = await c_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(c_wallet);
        number_to_allocate = 6;
    ...
    let allocate_4 = await c_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
    ...
    let allocate_5 = await d_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(d_wallet);
        number_to_allocate = 10;
    ...
    //allocate 4 nfts should get 2
    let allocate__retry_1 = await a_wallet.try_sale_nft_allocation(
      Principal.fromActor(sale_canister),
      {
        principal = Principal.fromActor(a_wallet);
        number_to_allocate = 1;
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "can allocate non additive items",
          switch (allocate_3) {
            case (#ok(res)) {
              if (res.allocation_size == 2) {
        ...
        S.test(
          "fail if no nfts available to allocate",
          switch (allocate_5) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 5004) {
        ...
        S.test(
          "can do a new allocation after expiration",
          switch (allocate__retry_1) {
            case (#ok(res)) {
              if (res.allocation_size == 1) {
  ...
  public shared func testManagement() : async { #success; #fail : Text } {
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "fail if canister date set low",
          switch (set_canister_sale_open_date_low) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 16) {
                //
        ...
        S.test(
          "fail if canister registration set low",
          switch (set_canister_registration_date_low) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 16) {
                //
        ...
        S.test(
          "fail if canister end date set low",
          switch (set_canister_end_date_low) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 16) {
                //
        ...
        S.test(
          "fail if canister sale date set high",
          switch (set_canister_sale_open_date_high) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 16) {
                //
        ...
        S.test(
          "fail if registration date set high",
          switch (set_canister_registration_date_high) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 16) {
                //
        ...
        S.test(
          "fail if end date set high",
          switch (set_canister_end_date_high) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 16) {
                //
  ...
  public shared func testLoadNFTS() : async { #success; #fail : Text } {
    ...
    let add_minted_unauth = await a_wallet.try_sale_manage_nft(
      Principal.fromActor(sale_canister),
      [
        #add({
          canister = Principal.fromActor(canister);
    ...
    let suite = S.suite(
      ...
      [
        ...
        S.test(
          "fail if non owner adds nfts",
          switch (add_minted_unauth) {
            case (#ok(res)) { "unexpected success" # debug_show (res) };
            case (#err(err)) {
              if (err.number == 2000) {
                //
        ...
        S.test(
          "can add unminted items",
          switch (add_unminted_1) {
            case (#ok(res)) {
        ...
        S.test(
          "cant readd items",
          switch (add_unminted_1_second_time) {
            case (#ok(res)) {
              if (res.total_size == 4 and res.items.size() == 2) {
                switch (res.items[0]) {
                  case (#err(err)) {
        ...
        S.test(
          "can get inventory item",
          switch (specific_inventory) {
            case (#ok(res)) {
              if (res.token_id == "1") {
                "expected success";
              } else {

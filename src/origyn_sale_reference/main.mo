...
shared (deployer) actor class SaleCanister(__initargs : Types.InitArgs) = this {
        ...
        case (#add(val)) {
          ...
          switch (Map.get<Text, Types.NFTInventoryItem>(state.nft_inventory, Map.thash, val.token_id)) {
            case (null) {
              ...
              Map.set<Text, Types.NFTInventoryItem>(
                state.nft_inventory,
                Map.thash,
                val.token_id,
                {
        ...
        case (#update(val)) {
          ...
          switch (Map.get<Text, Types.Group>(state.nft_group, Map.thash, val.namespace)) {
            ...
            case (?found) {
              ...
              found.tier := val.tier;
              switch (val.members) {
                //if provided replaces the members
                case (null) {};
                case (?members) {
                  var tree = Map.new<Principal, Int>();
                  for (this_item in members.vals()) {
        ...
        switch (Map.get<?TokenSpec, tracker>(token_map, hash_null_token, thisPricingToken)) {
          ...
          case (?existing_map) {
            ...
            switch (existing_allowed_amount, aGroup.allowed_amount) {
              ...
              case (?new, ?old) {
                let thisPricingAmount = switch (thisPricing) {
                  case (#free) { 0 };
                  case (#cost_per(detail)) { detail.amount };
                };
  ...
  public shared (msg) func allocate_sale_nft_origyn(request : Types.AllocationRequest) : async Result.Result<Types.AllocationResponse, Types.OrigynError> {
    ...
    if (request.number_to_allocate > 50) {
      //temporary...only allow purchasing 50 at a time
  ...
  public shared (msg) func redeem_allocation_sale_nft_origyn(request : Types.RedeemAllocationRequest) : async Result.Result<Types.RedeemAllocationResponse, Types.OrigynError> {
    ...
    let nft_gateway : NFTTypes.Service = switch (state.nft_gateway) {
      case (null) {
        return #err(Types.errors(#bad_config, "redeem_allocation_sale_nft_origyn - bad gateway config null", ?msg.caller));
      };
    ...
    let remove_specs = Array.filter<(?TokenSpec, ?Nat, [(Nat, ?Nat)])>(
      user_info.prices,
      func(item) {
        switch (item.0) {
          case (null) { true }; //free
    ...
    for (this_item in flat_price.vals()) {
      ...
      let this_number = switch (this_item.1) {
        case (null) {
          //this means the user has reached a price where they can allocate up to as many as they want
  ...
  /**
    ...
    */
  private func get_possible_purchases(caller : Principal, token : ?TokenSpec, number_to_allocate : Nat) : {
    user_info : {
      prices : [(?TokenSpec, ?Nat, [(Nat, ?Nat)])]; //token, max_allowed, (amount, number)
      personal_reservations : ([Types.Reservation], Nat, Nat);
      group_reservations : ([Types.Reservation], Nat, Nat);
  ...
  public shared (msg) func register_escrow_sale_nft_origyn(request : Types.RegisterEscrowRequest) : async Result.Result<Types.RegisterEscrowResponse, Types.OrigynError> {
    ...
    let current_reg = switch (request.escrow_receipt) {
      ...
      case (?val) {
        ...
        let nft_canister : NFTTypes.Service = switch (state.nft_gateway) {
          case (null) {
            return #err(Types.errors(#bad_config, "register_escrow_sale_nft_origyn - no gateway ", ?msg.caller));
          };
          case (?val) { actor (Principal.toText(val)) };
        };
        ...
        let balance = switch (await nft_canister.balance_of_nft_origyn(#principal(msg.caller))) {
          case (#err(err)) {
            return #err(Types.errors(#improper_escrow, "register_escrow_sale_nft_origyn - error checking balance " # debug_show (request, err), ?msg.caller));
          };
          case (#ok(val)) { val };
        };
    ...
    return (#ok({ allocation = Iter.toArray<Types.RegisterEscrowAllocationDetail>(iter2); max_desired = current_reg.max_desired; escrow_receipt = request.escrow_receipt; allocation_size = current_reg.allocation_size; principal = current_reg.principal }));
  ...
  public query func get_inventory_sale_nft_origyn(start : ?Nat, size : ?Nat) : async Result.Result<Types.GetInventoryResponse, Types.OrigynError> {
    ...
    var size_requested = switch (size) {
      case (null) { Map.size(state.nft_inventory) };
      case (?val) { val };
    };
  ...
  public query (msg) func get_allocation_sale_nft_origyn(principal : Principal) : async Result.Result<Types.AllocationResponse, Types.OrigynError> {
    ...
    switch (Map.get<Principal, Types.Allocation>(state.user_allocations, Map.phash, principal)) {
      case (?val) {
        ...
        return (#ok({ allocation_size = val.nfts.size(); token = val.token; principal = val.principal; expiration = val.expiration }));
      };
      case (null) {
  ...
  public query (msg) func get_registration_sale_nft_origyn(principal : Principal) : async Result.Result<Types.RegisterEscrowResponse, Types.OrigynError> {
    ...
    switch (Map.get<Principal, Types.Registration>(state.user_registrations, Map.phash, principal)) {
      case (?val) {
        ...
        let iter2 = Iter.map<(Text, Types.RegistrationClaim), Types.RegisterEscrowAllocationDetail>(iter1, Types.stabalize_xfer_RegisterAllocation);
        return (#ok({ allocation = Iter.toArray<Types.RegisterEscrowAllocationDetail>(iter2); max_desired = val.max_desired; escrow_receipt = val.escrow_receipt; allocation_size = val.allocation_size; principal = val.principal }));
      };
      case (null) {
        return (#ok({ allocation = []; max_desired = 0; escrow_receipt = null; allocation_size = 0; principal = principal }));
      };
    };
